...
function ViewGrocery() {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const updated: IGrocery = result.data;

      // Real-time update: preserve existing image if API didn't return a new one
      setGroceries((prev) =>
        prev?.map((g) =>
          g._id === updated._id
            ? {
                ...updated,
                image: updated.image || g.image, // fallback to old image if empty
              }
            : g,
        ),
      );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Real-time remove from list without refresh
      setGroceries((prev) => prev?.filter((g) => g._id !== editing._id));
  ...
  hover:bg-red-700 transition disabled:opacity-60 disabled:cursor-not-allowed"
                  ...
                  disabled={loading}
                >
                  {loading ? "Deleting..." : "Delete Grocery"}
                </button>
              </div>
            </motion.div>
          </motion.div>
